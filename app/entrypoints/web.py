...
def verify_debug_credentials(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    if not config.debug_username or not config.debug_password:
        raise IntentKitAPIError(
            status_code=500,
            key="DebugCredentialsNotConfigured",
    ...
    * `404` - Agent not found
    ...
    if user_id:
        for message in messages:
            ...
            raise IntentKitAPIError(
                status_code=403,
                key="ChatNotBelongsToUser",
                message="Chat not belongs to user",
            )
...
async def retry_chat(
    ...
    * `500` - Internal server error
    ...
    if not last:
        raise IntentKitAPIError(
            status_code=404, key="no_messages_found", message="No messages found"
        )
...
async def delete_chat(
    ...
    * `404` - Agent or chat not found
    ...
    if not agent:
        raise IntentKitAPIError(
            status_code=404, key="agent_not_found", message="Agent not found"
        )
