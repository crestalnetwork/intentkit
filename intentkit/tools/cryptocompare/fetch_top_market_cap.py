...
from intentkit.tools.cryptocompare.base import CryptoCompareBaseTool, CryptoCurrency
        ...
        **kwargs,
        ...
        try:
            ...
            if "error" in market_cap_data:
                raise ToolException(market_cap_data["error"])
            ...
            logger.error("Error fetching top market cap: %s", e)
            raise ToolException(f"Failed to fetch top market cap: {e!s}")
