...
class CrwCrawlInput(BaseModel):
    """Input for fastCRW crawl tool."""
...
class CrwCrawl(CrwBaseTool):
    """Tool for crawling entire websites using fastCRW.

    This tool uses fastCRW's API to crawl websites and extract content from multiple pages.
    ...
    description: str = (
        ...
        "Optionally indexes content for querying via crw_query_indexed_content."
    )
    price: Decimal = Decimal("100")
    args_schema: ArgsSchema | None = CrwCrawlInput
    ...
    async def _arun(
        ...
        **kwargs,
        ...
        """Implementation of the fastCRW crawl tool.
        ...
        logger.debug("crw_crawl: Running crawl with context %s", context)
        ...
        # Get the API key from the agent's configuration. fastCRW self-host may
        # run without auth, so an absent key is allowed.
        ...
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"crw_crawl: Error from fastCRW API: {response.status_code} - {response.text}"
                    )
                    raise ToolException(
                ...
                if not crawl_id:
                    raise ToolException(
                        "Error: No crawl ID returned from fastCRW API"
                    )
                ...
                while poll_count < max_polls:
                    ...
                    status_response = await client.get(
                        f"{api_url}/v1/crawl/{crawl_id}",
                        headers=headers,
                    )

                    if status_response.status_code != 200:
                        logger.error(
                            f"crw_crawl: Error checking crawl status: {status_response.status_code} - {status_response.text}"
                        )
                        raise ToolException(
                    ...
                    if status == "completed":
                        ...
                        if index_content and pages_data:
                            try:
                                # Import indexing utilities from crw utils
                                ...
                                if agent_id and documents:
                                    ...
                                    new_metadata = (
                                        CrwMetadataManager.create_url_metadata(
                                            urls, documents, "crw_crawl"
                                        )
                                    )
                                    ...
                                    logger.info(
                                        f"crw_crawl: Successfully indexed {len(documents)} pages with {total_chunks} total chunks"
                                    )
                                else:
                                    formatted_result += "\n## Content Indexing\n"
                                ...
                                logger.error(
                                    f"crw_crawl: Error indexing content: {index_error}"
                                )
                                formatted_result += "\n## Content Indexing\n"
                        ...
                        logger.debug(
                            f"crw_crawl: Crawl in progress: {completed}/{total} pages"
                        )
                        ...
                        # Unknown status
                        logger.warning(
                            f"crw_crawl: Unknown crawl status: {status}"
                        )
                        await asyncio.sleep(5)
                        poll_count += 1
            ...
            logger.error("crw_crawl: Timeout crawling URL: %s", url)
            raise ToolException(
                f"Timeout error: The request to crawl {url} took too long to complete."
            )
        except Exception as e:
            logger.error("crw_crawl: Error crawling URL: %s", e, exc_info=True)
