...
class CrwScrape(CrwBaseTool):
    """Tool for scraping web pages using fastCRW with REPLACE behavior.

    This tool uses fastCRW's API to scrape web pages and REPLACES any existing
    ...
    args_schema: ArgsSchema | None = CrwScrapeInput
    ...
    async def _arun(
        ...
        **kwargs,
        ...
        """Implementation of the fastCRW scrape tool.
        ...
        logger.debug("crw_scrape: Running scrape with context %s", context)
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout / 1000 + 10) as client:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"crw_scrape: Error from fastCRW API: {response.status_code} - {response.text}"
                    )
                    raise ToolException(
                ...
                if index_content and result_data.get("markdown"):
                    try:
                        ...
                        if agent_id:
                            ...
                            vs_manager = CrwVectorStoreManager()
                            ...
                            split_docs = CrwDocumentProcessor.split_documents(
                            ...
                            if existing_vector_store:
                                ...
                                try:
                                    ...
                                    logger.info(
                                        f"crw_scrape: Preserving {len(preserved_docs)} docs from other URLs, "
                            ...
                            existing_metadata = await self.get_agent_tool_data_raw(
                                "crw", metadata_key
                            )
                            ...
                            logger.info(
                                f"crw_scrape: Successfully replaced content for {url} with {len(split_docs)} chunks"
                            )
                        else:
                            formatted_result += "\n## Content Indexing\n"
                        ...
                        logger.error(
                            f"crw_scrape: Error indexing content: {index_error}"
                        )
                        formatted_result += "\n## Content Indexing\n"
            ...
            logger.error("crw_scrape: Timeout scraping URL: %s", url)
            raise ToolException(
            ...
            logger.error("crw_scrape: Error scraping URL: %s", e, exc_info=True)
