...
It should have at least 0.01 USDC on Base Mainnet for payment.
The x402 payment is gas free, so you Don't need to worry about gas fees.
...
async function main() {
  try {
    const response = await queryAgent('example', 'What is the current price of Bitcoin?');
...
There is a skill in category `x402` that allows you to access other resources through X402.
